...
export function SortedData(){
	...
	for(var i in teams){
		...
		for(var j in teams[i].matches){
			...
			teleScaleScored += teams[i].matches[j].teleSwitchScored;
			teleVaultScored += teams[i].matches[j].teleSwitchScored;

			teleSwitchAttempted += teams[i].matches[j].teleSwitchScored;
			teleScaleAttempted += teams[i].matches[j].teleSwitchScored;
			



			//PRESTO FUEAL
			if(teams[i].matches[j].teleSwitchScored === "PrestoGear"){
				PrestoGear += 1;
			}
