...
export function SortedData(){
	...
	for(var i in teams){
		...
		var teleVaultSpeed = 0;

		var teleSwitchMatches = 0;
		var teleScaleMatches = 0;
		var teleVaultMatches = 0;
...
////////////////////////////////////////SWITCHS PEED///////////////////////////////////////////////
			...
			else if(teams[i].matches[j].teleSwitchSpeed === "switchVerySlow"){
			...
			else if(teams[i].matches[j].teleScaleSpeed === "scaleVerySlow"){
				teleScaleSpeed += 0.25;
				teleScaleMatches += 1;
			}

			////////////////////////////////VAULT SPEED//////////////////////////////
			if(teams[i].matches[j].teleVaultSpeed === "vaultFast"){
				teleVaultSpeed += 1;
				teleVaultMatches += 1;
			}

			else if(teams[i].matches[j].teleVaultSpeed === "vaultMedium"){
				teleVaultSpeed += 0.75;
				teleVaultMatches += 1;
			}

			else if(teams[i].matches[j].teleVaultSpeed === "vaultSlow"){
				teleVaultSpeed += 0.5;
				teleVaultMatches += 1;
			}

			else if(teams[i].matches[j].teleVaultSpeed === "vaultVerySlow"){
				teleVaultSpeed += 0.25;
				teleVaultMatches += 1;
			}
			...
			if(teams[i].matches[j].teleClimb === "true"){
				climbSuccessful += 1;
			}
			else{
				climbAttempted += 1;
...
////////////////////////////////////////SWITCHS PEED///////////////////////////////////////////////
		...
		var SwitchSpeed = 0;
		SwitchSpeed = round((teams[i].averages.teleSwitchSpeed/teams[i].averages.teleSwitchMatches)*100, 1);
		
		var ScaleSpeed = 0;
		ScaleSpeed = round((teams[i].averages.teleScaleSpeed/teams[i].averages.teleScaleMatches)*100, 1);

		var VaultSpeed = 0;
		VaultSpeed = round((teams[i].averages.teleVaultSpeed/teams[i].averages.teleVaultMatches)*100, 1);
		...
		var defenseRating = 0;
		defenseRating = round((teams[i].averages.defenseRating / (teams[i].averages.matches)), 1);

		var evasionRating = 0;
		evasionRating = round((teams[i].averages.evasionRating / (teams[i].averages.matches)), 1);
...
export function TeamGraph(teamtofind){
	...
	EndGameTotal = ['End-Game']
	MatchTotal = []

	MatchTotal.push(AutoPointsTotal,TelePointsTotal,EndGameTotal)
...
//----------------------------------------------------------------ALL GRAPH DATA---------------------------------------------------------\\
	...
	for(var i in DataTeamSearched){
		...
		//TELE
		if(DataTeamSearched[i].autoSwitchCube === "cubeSwitch1"){
			AutoPoints += 10;
			console.log("spagedsnm,fdsbnmfdsbt")
		}

		if(DataTeamSearched[i].autoSwitchCube === "cubeSwitch2"){
			AutoPoints += 20;
		}

		if(DataTeamSearched[i].autoScaleCube === "cubeScale1"){
			AutoPoints += 10;
		}

		if(DataTeamSearched[i].autoScaleCube === "cubeScale2"){
			AutoPoints += 20;
		}

		if(DataTeamSearched[i].baselineCrossed === true){
			AutoPoints += 10;
		}

		// TELE
		TelePoints += DataTeamSearched[i].teleScaleScored*5

		TelePoints += DataTeamSearched[i].teleSwitchScored*5

		TelePoints += DataTeamSearched[i].teleVaultScored*5

		//end game

		if(DataTeamSearched[i].teleClimb === true){
			// IF THEY WERE SUCCESSFUL
			if(DataTeamSearched[i].endgame === "climbOtherTeam"){
			EndGamePoints += 15;
			}

			if(DataTeamSearched[i].endgame === "climbOne"){
				EndGamePoints += 30;
			}

			if(DataTeamSearched[i].endgame === "climbTwo"){
				EndGamePoints += 60;
			}

			if(DataTeamSearched[i].endgame === "climbThree"){
				EndGamePoints += 90;
			}
		}
		...
		if(DataTeamSearched[i].endgame === "parkYes"){
			EndGamePoints += 5;
		}	
		...
		TelePointsTotal.push(TelePoints);

		AutoPointsTotal.push(AutoPoints);

		EndGameTotal.push(EndGamePoints);
