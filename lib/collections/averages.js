...
export function Teamlookup(teamtofind){
	...
	for(var i in CurrentTeam){
		//makes a string that has all the match numbers in them so we can check if it has been submitted before
		CurrentMatch = "match"+CurrentTeam[i].match
		
		//if the current checking match has been submitted before then 
		if(Matches.indexOf(CurrentMatch) >= 0){

			DuplicateMatches = (DuplicateMatches + CurrentMatch)
