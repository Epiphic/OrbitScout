...
////////////////////////////////////////SWITCHS PEED///////////////////////////////////////////////
		...
		var ScaleDropAvg = 0;
		ScaleDropAvg = round(((teams[i].averages.teleScaleAttempted - teams[i].averages.teleScaleScored) / teams[i].averages.teleScaleAttempted), 1);

		var SwitchDropAvg = 0;
		SwitchDropAvg = round(((teams[i].averages.teleSwitchAttempted - teams[i].averages.teleSwitchScored) / teams[i].averages.teleSwitchAttempted), 1);

		var SwitchSpeed = 0;
		SwitchSpeed = round((teams[i].averages.teleSwitchSpeed/teams[i].averages.matches), 1);
		
		var ScaleSpeed = 0;
		ScaleSpeed = round((teams[i].averages.teleScaleSpeed/teams[i].averages.matches), 1);

		var climbSpeed = 0;
		climbSpeed = round((teams[i].averages.climbSpeed/teams[i].averages.matches), 1);

		var climbSuccessRate = 0;
		climbSuccessRate = round((teams[i].averages.climbSuccessful/(teams[i].averages.climbSuccessful+teams[i].averages.climbAttempted)), 1);

		var VaultCubeAvg = 0;
		VaultCubeAvg = round((teams[i].averages.vaultScored / teams[i].averages.matches), 1);

		var AutoSwitchAvg = 0;
		AutoSwitchAvg = round((teams[i].averages.autoSwitchScored / teams[i].averages.matches), 1);

		var AutoScaleAvg = 0;
		AutoScaleAvg = round((teams[i].averages.autoScaleScored / teams[i].averages.matches), 1);
		
		var teleDiedAvg = 0;
		teleDiedAvg = round((teams[i].averages.teleDied / teams[i].averages.matches), 1);
		...
		AutoSwitchSuccessAvg = round((teams[i].averages.autoSwitchSuccess / teams[i].averages.matches)*100, 1);

		var autoScaleSuccessAvg = 0;
		AutoScaleSuccessAvg = round((teams[i].averages.autoScaleSuccess / teams[i].averages.matches)*100, 1);
	...
	//======================================================USE THE TOTAL STATS AND RECENT STATS TO FIND CHANCE TO HIT USING BOTH.=====================================
		...
		var ChancetoHit_AutoSwitch = round(((AutoSwitchAvg*0.4) + (AutoSwitchAvgREC*0.6)), 1);

		var ChancetoHit_AutoScale = round(((AutoScaleAvg*0.4) + (AutoScaleAvgREC*0.6)), 1);
